...
app.controller("shoesControllerPublic", ($scope, $http) => {
    ...
    $scope.LastCartId = ()=> {
        $http.get(`http://localhost:3333/api/v1/cart/${JSON.parse(localStorage.getItem('user')).id}/last`, {
            headers: {
    ...
    $scope.countItems = (id) => {
        $http.get(`http://localhost:3333/api/v1/cart/items/${localStorage.getItem('cartID') ? localStorage.getItem('cartID') : id}`, {
            headers: {
                "Authorization": "Bearer " + localStorage.getItem("token")
            }
        }).then((response) => {
    ...
    $scope.createCart = () => {
        $http.post("http://localhost:3333/api/v1/cart", {
    ...
    $scope.addToCart = (id, price) => {
        ...
        for (let i = 0; i < userDetails.length; i++) {
            ...
            if (isLastCartClosed) {
                ...
                $http.post("http://localhost:3333/api/v1/cartItem", {
                    productId: id,
