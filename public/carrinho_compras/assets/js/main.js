const app = angular.module("appCarrinho", [])


app.controller("controllerCarrinho", ($scope, $http) => {
    $scope.cartItem = [];
    $scope.cart = [];
    $scope.total = 0;

    $scope.closeCart = () => {
        $http.put(`http://localhost:3333/api/v1/cart/finish/${localStorage.getItem('cartID')}`, null, {
            headers: {
    ...
    $scope.getCartById = () => {
        $http.get(`http://localhost:3333/api/v1/cart/${localStorage.getItem('cartID')}`, {
            headers: {
                "Authorization": "Bearer " + localStorage.getItem("token"),
            }
        })
    ...
    $scope.updateTotalPriceCart = () => {
        $http.put(`http://localhost:3333/api/v1/cart/${localStorage.getItem('cartID')}`, {
    ...
    $scope.updateQuantity = (id, quantity, price) => {
        $http.put(`http://localhost:3333/api/v1/cartItem/${id}`, {
            quantity
        }, {
            headers: {
                "Authorization": "Bearer " + localStorage.getItem("token"),
    ...
    $scope.deleteItemFromCart = (id) => {
        $http.delete(`http://localhost:3333/api/v1/cartItem/${id}`, {
            headers: {
                "Authorization": "Bearer " + localStorage.getItem("token"),
            }
        }).then((response) => {
